...
export class Paginator extends HTMLElement {
    ...
    #oldView = null
    #createView(keepOld = false) {
        if (this.#view) {
            if (keepOld) {
                // fleur-epub patch: 滚动模式章节接力时保留旧视图，待新章节渲染完成后再移除，避免闪屏
                if (this.#oldView) {
                    this.#oldView.destroy()
                    this.#container.removeChild(this.#oldView.element)
                }
    ...
    #beforeRender({ vertical, rtl, background }) {
        ...
        this.#background.style.setProperty('background', background)
    ...
    #afterScroll(reason) {
        ...
        if (this.scrolled) detail.fraction = this.start / this.viewSize
            ...
            this.#header.style.setProperty('visibility', page > 1 ? 'visible' : 'hidden')
    ...
    async #display(promise) {
        ...
        if (src) {
            ...
            this.#view = view
            // fleur-epub patch: 新章节渲染完成后移除旧视图（同一帧内完成换装 + 滚动定位，不产生闪屏）
            const oldView = this.#oldView
            this.#oldView = null
            if (oldView) {
                this.#container.removeChild(oldView.element)
                oldView.destroy()
            }
        }
